...
async function test() {
    ...
    const user = await User.findOne({ email: 'chetan.dubliyacse2025@indoreinstitute.com' });
    ...
    try {
        const res = await axios.post('https://campuspulse-jnfo.onrender.com/api/upload', formData, {
            headers: {
                ...formData.getHeaders(),
